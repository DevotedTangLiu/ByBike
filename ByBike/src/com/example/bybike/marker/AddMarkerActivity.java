...
public class AddMarkerActivity extends AbActivity {
    ...
    private void addMarker(){
        ...
        if(!SharedPreferencesUtil.getSharedPreferences_b(AddMarkerActivity.this, Constant.ISLOGINED)){
            ...
            showDialog("温馨提示","您还未登录或者登录已过期，请重新登录后再试", new DialogInterface.OnClickListener() {
...
//        p.put("price", "");
//        p.put("allowEnter", "");
        p.put("img1", photoFiles[0], "multipart/form-data");
        if(photoFiles[1] != null){
            p.put("img2", photoFiles[1], "multipart/form-data");
        }
        
        if(photoFiles[2] != null){
            p.put("img3", photoFiles[2], "multipart/form-data");
        }
        if(photoFiles[3] != null){
            p.put("img4", photoFiles[3], "multipart/form-data");
        }
        // 绑定参数
     ...
     */
    private void goBack() {
        showDialog("温馨提示", "确认取消并退出本页面吗？退出后本友好点信息将不再保存。", new DialogInterface.OnClickListener() {
